...
from DN_train_npy import cudataset
from DN_test_npy import cudatatest
...
class Trainer:
    def __init__(self):
        ...
        if os.path.exists('model/DN/latest.pth'):
            print('===> Loading pre-trained model...')
            state = torch.load('model/DN/latest.pth')
    ...
    def train(self):
        ...
        for ep in range(1, self.epoch + 1):
            ...
            if not os.path.exists("model/DN/"):
                os.makedirs("model/DN/")
            torch.save(state, os.path.join('model/DN/', 'latest.pth'))
            torch.save(state, os.path.join('model/DN/', str(ep) + '.pth'))
            matplotlib.use('Agg')
            plot_loss_list = self.train_loss
            fig1 = plt.figure()
            plt.plot(plot_loss_list)
            plt.savefig('DN_x4_loss.png')
            ...
            plt.savefig('DN_x4_psnr.png')
            fig3 = plt.figure()
            plt.plot(self.val_ssim)
            plt.savefig('DN_x4_ssim.png')
            plt.close('all')
    ...
    def test(self, i):
        ...
        with torch.no_grad():
            ...
            for batch_test, (lr, hr, rgb) in enumerate(test_loader):
                ...
                dic[lr_img[batch_test] + '_dn'] = save_mat(DN)
                dig[lr_img[batch_test] + '_gt'] = save_mat(hr)

                DN = data_process_npy(DN, 0, batch_test)
                hr = data_process_npy(hr, 0, batch_test)
                

                PSNR.append(psnr.psnr(DN, hr))
                SSIM.append(psnr.SSIM(DN, hr))
            ave_psnr = np.mean(PSNR)
            ave_ssim = np.mean(SSIM)
            if not os.path.exists('result/DN/MAT/'):
                os.makedirs('result/DN/MAT/')
            if ave_psnr > self.best_psnr:
                self.best_psnr = ave_psnr
                self.best_model = i
                sio.savemat('result/DN/MAT/best.mat', dic)
                sio.savemat('result/DN/MAT/gt.mat', dig)

                torch.save(state,os.path.join('model/DN/', 'best.pth'))
            sio.savemat('result/DN/MAT/'+str(i)+'.mat', dic)
