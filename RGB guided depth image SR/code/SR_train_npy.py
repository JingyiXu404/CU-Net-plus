...
class cudataset(data.Dataset):
    def __init__(self,target_name,guide_name,label_name):
        ...
        self.depth = np.load('trainset/SR/'+target_name+'.npy')#(batch,height,width,c)
        self.depth = np.transpose(self.depth, (0, 3, 1, 2))
        self.depth_t = torch.from_numpy(self.depth)

        self.gt = np.load('trainset/SR/'+label_name+'.npy')  # (batch,height,width,c)
        self.gt = np.transpose(self.gt, (0, 3, 1, 2))
        self.gt_t = torch.from_numpy(self.gt)

        self.rgb = np.load('trainset/SR/'+guide_name+'.npy')  # (batch,height,width,c)
